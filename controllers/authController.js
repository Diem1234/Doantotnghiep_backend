...
export const createMember = async (req, res, next) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Đã xảy ra lỗi khi thêm thành viên vào tài khoản" });
  }
};
    ...
    const results = await Account.find(conditionFind, {
      ...
      familyMembers: {
        $filter: {
          ...
          as: "member",
          cond: {
            $regexMatch: { $toString: "$$member.name" },
...
export const memberStatusFilter = async (req, res, next) => {
  try {
    ...
    const membersWithStatus = accounts.flatMap((account) =>
      account.familyMembers.filter((member) => member.status === status)
    );
    const membersWithStatusCount = membersWithStatus.length;

    res.send({
      code: 200,
      count: membersWithStatusCount,
      payload: membersWithStatus,
    });
  } catch (error) {
...
export const memberStatusTwo = async (req, res, next) => {
  try {
    ...
    const membersWithAbnormalStatus = accounts.flatMap((account) =>
      account.familyMembers.filter((member) => member.status !== "Bình Thường")
    );
...
export const deleteFamilyMember = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "An error occurred while deleting the family member" });
  }
};
export const updateFamilyMember = async (req, res) => {
  try {
    ...
    const index = account.familyMembers.findIndex(
    ...
    return res.status(200).json({
      message: "Cập nhật family member thành công",
      payload: account.familyMembers[index],
    });
  } catch (error) {
    console.error(error);
    return res
      .status(500)
      .json({ message: "Đã xảy ra lỗi khi cập nhật family member" });
  }
};
...
export const getSuggest = async (req, res, next) => {
  try {
    ...
    const apiResponse = await axios.get(
      "https://localhost:44327/api/Service",
      {
        params: apiParams,
    ...
    const matchedDishes = await Promise.all(
